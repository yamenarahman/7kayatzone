...
add_filter('mesmerize_override_with_thumbnail_image', function ($value) {
    ...
    if (isset($post) && $post->post_type === 'page') {
        $value = get_theme_mod('inner_header_show_featured_image', false);
...
function mesmerize_header_background_type_image_settings($section, $prefix, $group, $inner, $priority)
{
    ...
    if ( ! $inner) {
        ...
        mesmerize_add_kirki_field(array(
            'type'      => 'ope-info-pro',
            'label'     => esc_html__('Parallax header background image available in PRO. @BTN@', 'mesmerize'),
            'section'   => $section,
            'settings'  => $prefix . "_parallax_pro",
